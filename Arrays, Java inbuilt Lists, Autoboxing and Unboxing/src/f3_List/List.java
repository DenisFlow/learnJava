package f3_List;

import java.util.ArrayList;
import java.util.Scanner;

public class List {
    private static Scanner scanner = new Scanner(System.in);
    private static GroceryList gl = new GroceryList();

    public static void main(String[] args) {
        boolean quit = false;
        int choice = 0;




        printInstructions();

        while (!quit) {
            System.out.println("Enter your choice: ");
            choice = scanner.nextInt();
            scanner.nextLine();

            switch (choice) {
                case 0:
                    printInstructions();
                    break;
                case  1:
                    gl.printGroceryList();
                    break;
                case 2:
                    addItem();
                    break;
                case 3:
                    modifyItem();
                    break;
                case 4:
                    removeItem();
                    break;
                case 5:
                    searchItem();
                    break;
                case 6:
                    processArrayList();
                    break;
                case 7:
                    quit = true;
                    break;
            }
            printInstructions();
        }
    }

    private static void processArrayList() {
            ArrayList<String> newArray = new ArrayList<>();
            newArray.addAll(gl.getGroceryList());
            ArrayList<String> nextArrayList = new ArrayList<>(gl.getGroceryList());

            String[] myArray = new String[gl.getGroceryList().size()];
            myArray = gl.getGroceryList().toArray(myArray);
    }

    public static void printInstructions() {
        System.out.println("\nPress");
        System.out.println("\t 0 - To print choice options.");
        System.out.println("\t 1	-	To	print the	list	of grocery items.");
        System.out.println("\t 2	-	To	add an	item to	the list.");
        System.out.println("\t 3	-	To	modify	an	item	in the list.");
        System.out.println("\t 4	-	To	remove	an	item	from the list.");
        System.out.println("\t 5 - To search for an item in the list.");
        System.out.println("\t 6 - To quit the application.");
    }

    public static void addItem() {
        System.out.println("Please enter the grocery item: ");
        gl.addGroceryItem(scanner.nextLine());
    }

    public static void modifyItem() {
        System.out.println("Please enter old item: ");
        String item = scanner.nextLine();
        System.out.println("Enter new item: ");
        gl.replaceItem(item, scanner.nextLine());
    }

    public static void removeItem() {
        System.out.println("Please enter item for removal: ");
        String item = scanner.nextLine();
        gl.removeGroceryItem(item);
    }

    public static void searchItem() {
        System.out.println("Please enter searchig item: ");
        if (gl.findItem(scanner.nextLine()) != null)
            System.out.println("Item was found in grocery list");
        else
            System.out.println("Item not in the grocery list");

    }
}
